const DELETE_SCRIPT = function () {
  try {
    var s = document.getElementById('mwb-salesforce-delete-sync_data-select');
    if (!s) return { success: false, reason: 'Delete select not found — may not be a WC product page' };
    s.value = '81960';
    var b = document.getElementById('mwb-salesforce-delete-data-button');
    if (!b) return { success: false, reason: 'Delete button not found' };
    b.click();
    return { success: true };
  } catch (e) {
...
runBtn.addEventListener('click', async () => {
  ...
  const confirmed = confirm(
    `Pricebook Entries Meta Delete\n\nThis will run the Pricebook Entries meta delete on ${tabs.length} tab${tabs.length !== 1 ? 's' : ''} in this window.\n\nTabs will be processed sequentially with a ${delay}ms delay between each.\n\n⚠️ This action will delete Pricebook Entry meta data. Proceed?`
  );
  ...
  for (let i = 0; i < tabs.length; i++) {
    ...
    try {
      ...
      if (result && result.success) {
        addTabItem(tab, 'success', 'Deleted');
        successCount++;
      } else {
  ...
  setStatus('done', `Complete — ${successCount} deleted, ${skippedCount} skipped, ${errorCount} errors`);
  runBtn.disabled = false;
  runBtn.textContent = '↺ Run Again';

  alert(
    `Pricebook Entries Meta Delete Complete\n\n✓ Deleted: ${successCount}\n— Skipped (not a product page): ${skippedCount}\n✗ Errors: ${errorCount}\n\nTotal tabs processed: ${tabs.length}`
  );
});
