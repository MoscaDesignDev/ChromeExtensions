...
const SCAN_SCRIPT = function () {
  ...
  allItems.forEach(item => {
    ...
    if (titleDiv.textContent.trim().includes('PricebookEntry Feed')) {
...
scanBtn.addEventListener('click', async () => {
  ...
  for (const tab of tabs) {
    ...
    try {
      ...
      if (data && data.urls.length > 0) {
        ...
        renderTabItem(tab, 'missing', 'No PricebookEntry Feed links', false);
      }
    } catch (err) {
  ...
  if (totalUrls === 0) {
    setStatus('none', `No PricebookEntry Feed links found across ${tabs.length} tab${tabs.length !== 1 ? 's' : ''}`);
    countBadge.style.display = 'none';
    return;
  }

  setStatus('ready', `Found ${totalUrls} link${totalUrls !== 1 ? 's' : ''} across ${foundProducts.length} product${foundProducts.length !== 1 ? 's' : ''}`);
  countBadge.textContent = `${totalUrls} PricebookEntry Feed link${totalUrls !== 1 ? 's' : ''} ready to open`;
