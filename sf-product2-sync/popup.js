const PRODUCT2_SCRIPT = function () {
  try {
...
runBtn.addEventListener('click', async () => {
  ...
  const confirmed = confirm(
    `SF Product2 Sync\n\nThis will run the Product2 Feed sync on ${tabs.length} tab${tabs.length !== 1 ? 's' : ''} in this window.\n\nTabs will be processed sequentially with a ${delay}ms delay between each.\n\nProceed?`
  );
